...
class Unit extends BranthBehaviour {
	constructor(team, index, position, name, strength, iconIndex) {
		...
		this.team = team; // 0=army1, 1=army2
		...
		this.imageXScale = this.team > 0? -1 : 1;
		this.actionInterval = {
			min: 500,
			max: 1000
		};
	...
	render() {
		...
		Draw.strip("icons", this.iconIndex, this.x, this.iconDrawY);
	}
	alarm0() {
...
Boot.start = () => {
	...
	// Draw.addStrip(new Vector2(0.5, 1), "KnightDeath", "src/img/KnightDeath_strip.png", 15);
	...
	// Draw.addStrip(new Vector2(0.5, 1), "KnightRoll", "src/img/KnightRoll_strip.png", 15);
	// Draw.addStrip(new Vector2(0.5, 1), "KnightRun", "src/img/KnightRun_strip.png", 8);
	...
	if (!BS.inputHasLoaded) {
		...
		if (BS.debug) {
			...
			for (const i of OBJ.take(Unit)) {
				const unit = n[`army${i.team + 1}`][`s${i.index + 1}`];
				txt = `#${i.index + 1} ${i.name}\nstr: ${i.strength}/${unit.strength}\nicon: ${i.iconIndex}`;
		...
		if (Input.keyDown(KeyCode.Space)) {
			...
			for (let h = 0; h < 2; h++) {
				let txt = "";
				for (let i = 0; i < 5; i++) {
					const unit = n[`army${h + 1}`][`s${i + 1}`];
					OBJ.create(Unit, h, i, BS.Spot[spotCount++], unit.name, unit.strength, unit.iconIndex);
				}
			}
			BS.Battle.start();
		}
